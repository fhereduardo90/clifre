...
module.exports =  function(req, res, next) {
  ...
  return jwt.verify(token, app.get('jwtKey'), function(err, decoded) {
    if (err) return res.status(403).end(); //.json(new ApiError('Failed to authenticate token.', 403, [err.message]));
    return sequelize.User.findOne(
      ...
      .then(function(user){
        if (!user) return res.status(403).end(); //.json(new ApiError('Failed to authenticate token.', 403));
        req.user = user;
        next();
        return null;
      })
      .catch(function(err){
        return res.status(403).end(); //.json(new ApiError('Failed to authenticate token.', 403));
      });
  });
};
